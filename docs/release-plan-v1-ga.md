...
**✅確定（2026-06-14）**: 1号店のコア価値 = **本部集中ライブ監視 + BCP(Jアラート録画) がメイン**。録画再生(VOD)は付加価値（防犯の生命線=録画はi-PRO NVR本体が担う）。
→ これにより **approach B（VODをGAゲート外＝ファストフォロー）は完全に正当化**。VOD後回しは"防犯の欠落"ではなく"リモート確認の利便を後追加"。GAは「ライブ監視+BCP」を主商品として出す。
→ 価値訴求/契約も「多店舗本部の集中ライブ監視 + Jアラート自動録画」を主に据える（VOD単体価値を前借りしない）。
...
## 0.6 eng-review 差分再審査（2026-06-14・新スコープ）

CEOレビューで動いた新コードパスを再審査。技術的な実装条件を確定。

| # | 差分発見 | 決定 |
|---|---|---|
| DR1 | C1テナント次元authz契約テスト | **実DB統合テスト+テナント×ロール行列**。staging DB(WBS2.6)を先行＝Phase A依存。モック不可（RLS×コード不一致を捕えるため）。G1不可侵。 |
| DR2 | C2 QRエンロール（新規エンドポイント=攻撃面） | **単一使用・短期TTL・tenant/store束縛のエンロールトークン+冪等性**。共通長期キーは不可。 |
| DR3 | C3 計測の保存基盤 | **スキーマday1確定 + Supabaseテーブル/構造化ログ + 保持期限**。専用パイプラインは作らない(300同時時に移行)。 |
| DR4 | TC3 中断見える化の信号源 | **既存heartbeat/死活監視(WBS3.2)を単一の真実源**に。顧客表示はその派生。監視/録画を区別。社内アラートと食い違わせない。 |

**新規クリティカルギャップ（継続）**: Frigate除外で i-PRO grid snapshot が単一依存。失敗時UX（プレースホルダ+再試行）をPhase Bに明示。
