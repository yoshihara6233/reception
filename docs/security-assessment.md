...
| B4 | bootstrap から `supabase_service_role_key` の返却を撤廃 | **機構は実装済**（`edge_devices.scoped_only`・既定 false）。有効化は B3 の soak 後に1台ずつ |
...
**続き（2026-08-06・同日対応）**: 同ルートの**フレームごと `auth.getUser()`** を削減した。
判明したのは、往復が**2箇所**あったこと — ルートに加えて `src/middleware.ts` が
`/api/edges/**` にも掛かっており、1フレームごとに `getUser()` ＋ `admin_users` の1行引きを
していた（`/api/` は許可プレフィックスにも静的アセットにも当たらないため）。
`src/lib/auth/token-cache.ts` を両方から使い、アクセストークン単位で判定を30秒メモ化する。

- **キャッシュは実検証の代替ではない** — 判定を作るときは必ず `getUser()` を通す。
- **JWKS でのローカル検証は採らない**。往復はゼロになるがトークンの `exp`（約1時間）まで
  ログアウト・利用者削除が効かなくなる。TTL を自分で決められる方式なら失効の遅れを30秒に抑えられる。
- 鍵はトークンの SHA-256（生のトークンをメモリに残さない）。TTL とトークンの `exp` の
  **早い方**で切る。`exp` は署名検証せずに読むが、**キャッシュを短くする方向にしか使わない**ので安全側。
- ミドルウェアで `admin_users` を引けなかったときは**覚えない**（false を30秒持つと境界が効かない窓を作る）。

境界の判定そのものは不変。`middleware.test.ts` に「同一トークンの連続アクセスで `getUser` は1回」
「キャッシュ後も baggage_manager は遮断され続ける」を追加して CI で固定した。
