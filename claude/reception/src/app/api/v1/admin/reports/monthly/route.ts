...
export async function POST(req: NextRequest) {
  ...
  const { storeId, year, month } = body as {
    storeId: string | null
    year: number
    month: number
  }
  ...
  const dateTo = new Date(year, month, 1).toISOString()
  ...
  if (error) {
    console.error('[reports/monthly] Supabase error:', error)
  ...
  const fileName = encodeURIComponent(`来訪サマリー_${storeLabel}_${year}年${String(month).padStart(2, '0')}月.pdf`)
