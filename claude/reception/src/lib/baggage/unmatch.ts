...
export function buildUnmatchEmail(storeName: string, date: string, items: UnmatchItem[]): { subject: string; html: string } {
  ...
  const rows = items
    .map((i) => `<tr><td style="padding:6px 12px;border-bottom:1px solid #eee">${label(i.kind)}</td>`
      + `<td style="padding:6px 12px;border-bottom:1px solid #eee">${i.personLabel}</td>`
      + `<td style="padding:6px 12px;border-bottom:1px solid #eee">${i.at ?? '—'}</td></tr>`)
    .join('')
  const subject = `[手荷物検査] ${date} アンマッチ ${items.length}件（${storeName}）`
  const html = items.length === 0
    ? `<p>${date} の${storeName}に未処理のアンマッチはありませんでした。</p>`
    : `<p>${date} の${storeName}のアンマッチ ${items.length}件です。映像を再生してご確認ください。</p>`
