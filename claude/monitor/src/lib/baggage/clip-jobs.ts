 ...
 *
 * ここは I/O を持たない純関数のみ:
 *   - buildClipJobs: 検査セッションからカメラ毎のジョブ仕様を作る
 *       （検査窓 ±バッファ / not_before で誤報防止 / deadline で失敗確定）
 *   - validateClipReport: エッジ報告の健全性検査（尺80% / 時計ズレ閾値）
 *   - nextRetryAt / isPastDeadline: 指数バックオフと期限判定
 *
 * inspection_clip_jobs（M1）の not_before / deadline_at と対応。
 * エッジは edge_devices.device_token 認証で poll する（M5）。
 */
...
export interface ClipJobSettings {
  ...
  notBeforeMin?: number
  /** NVR の録画保持日数（deadline = 検査終了 + (これ-2)日）。inspection_settings.nvr_retention_days。 */
