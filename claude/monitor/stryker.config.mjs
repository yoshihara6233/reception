...
export default {
  ...
  /**
   * しきい値。**2026-08-09 の実測 80.17% を基準に置いた**もので、理想値ではない。
   *
   ...
   *
   * ── 経緯 ──────────────────────────────────────────────────────────
   * 導入時 64.38% → 3 モジュールの穴を埋めて 69.98%（break 65）
   *              → エッジ側 3 モジュールを埋めて 80.17%（break 75）
   *
   * 「no coverage」は **139 → 22** まで減った。残りはほぼ生き残り変異
   * （テストは通っているが殺せていない）で、性質が違う。
   *
   * 現状の内訳（低い順・次に手を入れるならこの順）:
   *   admin/super-admin-floor  70%  … 下限ガード（最後の1人を消させない）
   *   tenant/features.ts       70%  … オプション機能フラグ
   *   admin/user-scope.ts      73%
   *   jalert-poller/match.ts   74%  … 生き残り 38（no cov は 9）
   *   storage/edge-images-sign 76%
   */
